...
class SelectedCategoryState {
  ...
  SelectedCategoryState([this.value]);
}

class SearchRadiusState {
  double value;

  SearchRadiusState([this.value = 50000.0]);
}
...
final selectedCategoryProvider = Provider<SelectedCategoryState>((ref) {
  return SelectedCategoryState();
});

// Provides the search radius (default 50km)
final searchRadiusProvider = Provider<SearchRadiusState>((ref) {
  return SearchRadiusState();
});
...
final nearbyListingsProvider = FutureProvider<List<Listing>>((ref) async {
  ...
  final radius = ref.watch(searchRadiusProvider).value;
